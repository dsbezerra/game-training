...
draw_debug_draw_mixer(Vector2i dimensions) {
    ...
    for (Playing_Sound *sound = mixer.playing_sounds; sound != mixer.playing_sounds + array_count(mixer.playing_sounds); sound++) {
        ...
        //
        {
            ...
            immediate_quad(min, max, -5.f, played_color);
        }
        immediate_flush();
        ...
        //
        {
            ...
            sprintf(buf, "%s - Looping: %s - Pan: %.2f - Volume: %.2f\n", sound->name ? sound->name : "Unnamed", looping ? "true" : "false", sound->pan, sound->volume);
