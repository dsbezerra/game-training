...
opengl_init() {
    ...
    glGetIntegerv(GL_MAX_COLOR_TEXTURE_SAMPLES, &open_gl->max_multi_sample_count);
...
internal GLuint
framebuffer_tex_image(GLuint slot, u32 width, u32 height, GLint filter_type, GLuint format)
{
    GLuint result = 0;
    ...
    if (slot == GL_TEXTURE_2D_MULTISAMPLE) {
        open_gl->glTexImage2DMultisample(slot, open_gl->max_multi_sample_count, format,
                                         width, height,
                                         GL_FALSE);
    } else {
        glTexImage2D(slot, 0,
...
create_framebuffer(u32 width, u32 height, u32 flags, u32 color_buffer_count) {
    ...
    b32 has_depth    = flags & OpenGLFramebuffer_Depth;
    
    open_gl->glGenFramebuffers(1, &result.framebuffer_handle);
    open_gl->glBindFramebuffer(GL_FRAMEBUFFER, result.framebuffer_handle);
    ...
    GLenum status = open_gl->glCheckFramebufferStatus(GL_FRAMEBUFFER);
